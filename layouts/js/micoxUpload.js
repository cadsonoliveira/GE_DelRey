...
function removeX(quem){
 quem.parentNode.removeChild(quem);
}
function addEventX(obj, evType, fn){
...
*******/
 ...
 form = typeof(form)=="string"?$mX(form):form;
 ...
 if($mX(id_elemento_retorno)==null){ erro += "O elemento passado no 3o parâmetro não existe na página.\n";}
 if(erro.length>0) {
 ...
 /*var carregou = function() {
   removeEventX( $mX('micox-temp'),"load", carregou);
   var cross = "javascript: ";
   cross += "window.parent.$mX('" + id_elemento_retorno + "').innerHTML = document.body.innerHTML; void(0); ";

   $mX(id_elemento_retorno).innerHTML = html_erro_http;
   $mX('micox-temp').src = cross;
   //deleta o iframe
   setTimeout(function(){ removeX($mX('micox-temp'))}, 250);
  }
  */
...
window.document.getElementById( id_elemento_retorno ).innerHTML = iframeId.contentDocument.body.innerHTML;
 ...
 addEventX( $mX('micox-temp'),"load", carregou)
 ...
 if(html_exibe_carregando.length > 0){
  $mX(id_elemento_retorno ).innerHTML = html_exibe_carregando;
 }
