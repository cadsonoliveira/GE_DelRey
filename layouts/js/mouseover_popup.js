...
function showtrail(imagename,title,width,height){
	...
	w = width
	h = height
	timer = setTimeout("show('"+im+"',t,w,h);",200);
}
function show(imagename,title,width,height){
	...
	if( (navigator.userAgent.indexOf("Konqueror")==-1  || navigator.userAgent.indexOf("Firefox")!=-1 || (navigator.userAgent.indexOf("Opera")==-1 && navigator.appVersion.indexOf("MSIE")!=-1)) && (docwidth>650 && docheight>500)) {
		...
		newHTML = '<div class="border_preview" style="width:'+  width +'px;height:'+ height_aux +'px"><div id="loader_container"><div id="loader"><div align="center">Loading image preview...</div><div id="loader_bg"><div id="progress"> </div></div></div></div>';
	    newHTML = newHTML + '<h2 style="padding: 2px; 0px; 0px; 0px; margin: 0px; text-align:center;">' + ' '+title + '</h2>' 
		
    	newHTML = newHTML + '<div class="preview_temp_load"><img onload="javascript:remove_loading();" src="' + imagename + '" border="0"  width="'+(width-20)+'" height="'+(height-20)+'"></div>';
		newHTML = newHTML + '</div>'; 
